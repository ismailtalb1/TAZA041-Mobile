...
| أفكار الوجبات | `GET /customer/meal-suggestions`، `POST /customer/meal-suggestion` بصيغة `multipart/form-data` مع صورة اختيارية |

## قواعد التنفيذ

- جميع الطلبات المحمية ترسل `Bearer token` من التخزين الآمن.
- جلسات الموقع والموبايل مستقلة؛ تسجيل الدخول من عميل جديد لا يلغي العملاء الآخرين، وتُمدد صلاحية توكن الزبون النشط منزلقًا قبل انتهائها.
- لا تمسح استجابة `401` المتأخرة توكنًا أحدث، وتغيير كلمة المرور من التطبيق يحافظ على الجلسة التي نفذت التغيير ويلغي الجلسات الأخرى عبر الخادم.
...
- وقت الحجز في منتقي الساعة يعني اليوم الحالي فقط؛ لا يحوّل التطبيق الوقت الماضي تلقائيًا إلى الغد، ويعيد التحقق من كتالوج الطاولات قبل تثبيت الحجز.
...
- صفحة «عن المطعم» تعرض `working_hours` الأسبوعية من المصدر نفسه، بينما يعرض الملخص `today_hours`.
- شاشة محادثات الوجبة تعرض `quick_replies` و`suggested_items` كما يعيدها الخادم، وتحافظ على `conversation_id` بين خطوات المستشار.
- سجل أفكار الوجبات يتزامن في الواجهة فقط، وتفتح إشعارات `suggestion_id` الفكرة المرتبطة مباشرةً.
